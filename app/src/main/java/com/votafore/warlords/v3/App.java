...
public class App extends Application {
    ...
    public void onCreate() {
        ...
        Log.v(TAG, String.format(format1, prefix, "onCreate"));
        ...
        Observable.create(new ObservableOnSubscribe<Intent>() {
            ...
            public void subscribe(final ObservableEmitter<Intent> e) throws Exception {
                ...
                Log.v(TAG, String.format(format1, prefix, "SUBSCRIBER"));

                final BroadcastReceiver receiver = new BroadcastReceiver() {
                    @Override
                    public void onReceive(Context context, Intent intent) {
                        Log.v(TAG, String.format(format3, prefix, "SUBSCRIBER", "BROADCAST_RECEIVER", "onReceive"));
                        e.onNext(intent);
                    }
                };

                Log.v(TAG, String.format(format3, prefix, "SUBSCRIBER", "BROADCAST_RECEIVER", "created"));

                IntentFilter filter = new IntentFilter(WifiManager.NETWORK_STATE_CHANGED_ACTION);
                getApplicationContext().registerReceiver(receiver, filter);

                Log.v(TAG, String.format(format3, prefix, "SUBSCRIBER", "BROADCAST_RECEIVER", "registered"));

                e.setCancellable(new Cancellable() {
                    @Override
                    public void cancel() throws Exception {
                        getApplicationContext().unregisterReceiver(receiver);
                        Log.v(TAG, String.format(format3, prefix, "SUBSCRIBER", "BROADCAST_RECEIVER", "unregistered"));
                    }
                });
            }
            ...
            public void accept(Intent intent) throws Exception {
                Log.v(TAG, String.format(format3, prefix, "SUBSCRIBER", "BROADCAST_RECEIVER", "onNext received"));
                refreshIP();
            }
        });
    }
    ...
    public AdapterServerList getAdapter(){
        ...
        Log.v(TAG, String.format(format1, prefix, "getAdapter"));
